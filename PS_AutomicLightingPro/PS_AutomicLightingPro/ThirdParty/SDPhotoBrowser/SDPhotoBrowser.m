...
- (void)dealloc
{
    [showWindow removeObserver:self forKeyPath:@"frame"];
}
...
- (void)setupToolbars
{
    ...
    indexLabel.bounds = CGRectMake(0, 40, 80, 30);
...
//    ([[UIScreen mainScreen] bounds].size.height > 811 ? 44 :20)
    CGFloat _label_y =[[UIScreen mainScreen] bounds].size.height > 811 ? 44+16:20+16;
    _indexLabel.center =CGPointMake(self.bounds.size.width * 0.5,_label_y);
...
- (void)show
{
   ...
   showWindow = [windows lastObject];
    self.frame = showWindow.bounds;
    [showWindow addObserver:self forKeyPath:@"frame" options:0 context:nil];
    [showWindow addSubview:self];
    [showWindow bringSubviewToFront:self];
